"""
Usage:
python test_math.py --folder <folder path> --use-llm --llm-port 6767
"""
...
async def evaluate_folder(folder_path, verbose=False, client=None):
    ...
    Args:
        folder_path: Folder path containing JSON files
        verbose: Whether to print detailed information
        client: OpenAI client (for calling LLM)
    
    Returns:
        (total, correct, accuracy) tuple
    """
    ...
    if not json_files:
        print(f"Folder {folder_path} does not contain any JSON files (including subfolders)")
        return 0, 0, 0.0
