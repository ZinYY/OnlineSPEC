"""
EAGLE3 ENS (Exponential weighting on cumulative loss) pipeline.
...
def compute_weights_from_cumulative_losses(cumulative_losses: List[float], epsilon: float) -> Tuple[float, ...]:
    ...
    # p_i ∝ exp(-ε * cum_i) <=> softmax(-ε * cum_i) = softmax(-cum_i / (1/ε))
...
def evaluate(data_file, base_model_path, ea_model_path_1, ea_model_path_2, ea_model_path_3, 
    ...
    """Evaluate the EAGLE3 ensemble model (merge base learners with weights, then run inference)."""
...
def train(
    ...
    for i, (ea_model_path, ea_output_dir, lr, epochs) in enumerate(zip(ea_model_paths, ea_output_dirs, lrs, num_epochs_list), 1):
        ...
        print(f"\nBase learner {i}/3 (lr={lr}, epochs={epochs}) - train from own state:")
...
def run_pipeline(
    ...
    """
    print("=== EAGLE3 ENS pipeline execution started ===")
    ...
    for i, chunk_file in enumerate(chunk_files):
        ...
        if offline:
            ...
            print(f"  ├─ Training time (first base): {train_time:.2f} seconds")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='EAGLE3 ENS pipeline (base learners independent, merge at eval, weights from cumulative loss)')
    ...
    parser.add_argument('--lr-1', type=float, default=1e-3, help='Base learner 1 learning rate')
    parser.add_argument('--lr-2', type=float, default=1e-3, help='Base learner 2 learning rate')
    parser.add_argument('--lr-3', type=float, default=1e-3, help='Base learner 3 learning rate')
    parser.add_argument('--num-epochs', type=int, default=1, help='Training epochs (default for all models)')
    parser.add_argument('--num-epochs-1', type=int, default=None, help='Base learner 1 epochs')
    parser.add_argument('--num-epochs-2', type=int, default=None, help='Base learner 2 epochs')
    parser.add_argument('--num-epochs-3', type=int, default=None, help='Base learner 3 epochs')
    ...
    parser.add_argument('--offline', action='store_true', help='Offline mode: skip training')
    parser.add_argument('--debug-chunks', type=int, help='Debug: only process first N chunks')
    ...
    if not args.log_file:
        log_file = "results_eagle3_ens" + chunk_suffix + lr_epoch_suffix + time_suffix + ".json"
    else:
