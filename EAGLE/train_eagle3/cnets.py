...
from train_eagle3.modeling_llama_kv import LlamaForCausalLM
...
class Model(nn.Module):
    def __init__(self, config, ds_config, training_config, load_head=False, load_emb=True, path=None):
        ...
        try:
            if torch.cuda.is_bf16_supported():
                print("Using bfloat16 for target_model (better numerical stability)")
                dtype = torch.bfloat16
            else:
                print("bfloat16 not supported, using float32 for target_model")
                dtype = torch.float32
        except:
            print("Using float32 for target_model (for numerical stability)")
            dtype = torch.float32
    ...
    def scandata(self, datapath, tokenizerpath, cache_dir=None):
        """
        Build draft vocabulary mapping.
        If draft_vocab_size == vocab_size, skip frequency counting and use full vocabulary.
        
        Args:
            datapath: Path to training data
            tokenizerpath: Path to tokenizer
            cache_dir: Directory to save cache.pt (default: current directory)
        """
        ...
        # Fast path: if draft vocab size equals full vocab size, skip frequency counting
        if N >= self.vocab_size:
            print(f"Draft vocab size ({N}) >= full vocab size ({self.vocab_size})")
            print("Using full vocabulary (skipping frequency counting)...")
            # Identity mapping: use all tokens
            d2t = torch.arange(self.vocab_size)
        ...
        if not os.path.exists(cache_path):
            ...
            num_proc = 4  # Reduced for single GPU training to avoid resource contention
            ...
            def preprocess_function(examples):
                ...
                for i in range(len(examples['id'])):
                    ...
                    if isinstance(source[0], list):
                        ...
                        if roles.get(source[0].get("from", ""), "") != "user":
                            source = source[1:]
            ...
            print(f"Draft vocabulary cache saved to: {cache_path}")
        else:
